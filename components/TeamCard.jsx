...
const TeamCard = ({ name, role, photoUrl, frameUrl, socials = [] }) => {
  ...
  const shadow = useTransform(
    ...
    ([x, y]) =>
      `${-x * 50}px ${30 - y * 50}px 65px rgba(0, 0, 0, 0.8),
       0px 0px 30px rgba(225, 29, 39, 0.2)`
  );
    ...
    <div className="flex flex-col items-center w-full group">
      <div className="relative [perspective:1200px] w-full">
        ...
        >
          <motion.div
            ...
            style={{
              background: useTransform(
                [mouseXSpring, mouseYSpring],
                ([x, y]) =>
                  `radial-gradient(circle at ${50 + x * 100}% ${50 + y * 100}%,
                   rgba(255,0,0,0.2) 0%,
                   transparent 80%)`
              ),
            }}
          ...
          <div
            className="hidden md:flex absolute top-[105%] left-1/2 -translate-x-1/2 w-full max-w-[90%] text-center z-[10] flex-col items-center"
      ...
      <div className="flex md:hidden flex-col items-center text-center mt-6 z-20">
        ...
        <div className="flex gap-4">
          {safeSocials.map((link, idx) => (
            <SocialIcon key={idx} link={link} isMobile />
          ))}
        </div>
      </div>
    </div>
...
const SocialIcon = ({ link, isMobile }) => (
  <a
    ...
    } flex justify-center items-center rounded-lg bg-black/50 border border-white/10 backdrop-blur-md`}
    onClick={(e) => e.stopPropagation()}
  >
    {link?.type === "linkedin" ? (
      <Linkedin size={isMobile ? 18 : 14} color="#0077B5" fill="#0077B5" />
    ) : link?.type === "github" ? (
      <Github size={isMobile ? 18 : 14} color="#ffffff" />
    ) : (
